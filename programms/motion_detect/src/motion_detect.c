...
#include <swap.h>
#include <vga.h>
#include "../../camera/include/sobel.h"
...
uint8_t motDetect(uint8_t *old, uint8_t *new, uint32_t size)
{
  uint8_t motion = 0;
  for (uint32_t i = 0; i < size; i++)
  {
    //TODO: maybe have threshold
    if (old[i] != new[i])
    {
      motion = 1;
      break;
...
int main()
{
  ...
  uint8_t sobelMatrix[640 * 480];
  uint8_t oldSoberMatrix[640 * 480] = {0};
  ...
  while (1)
  {
    ...
    edgeDetection((uint8_t *)&grayscale[0], sobelMatrix, 640, 480, 100);

    if (motDetect(oldSoberMatrix, sobelMatrix, 640 * 480))
    {
