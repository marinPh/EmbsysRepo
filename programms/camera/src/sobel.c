                   ...
                   int32_t threshold)
{
  ...
  const int32_t gy_array[3][3] = {{1, 2, 1},
                                  {0, 0, 0},
                                  {-1, -2, -1}};
  int32_t valueX, valueY, tempx, tempy, result;
    ...
    for (int pixel = 1; pixel < width - 1; pixel++)
    {
      ...
      for (int dx = -1; dx < 2; dx++)
      {
        for (int dy = -1; dy < 2; dy++)
        {
          ...
          // TODO:change the following code to use the a ci
          //is ValueA and ValueB in the good order?
          asm volatile("l.nios_rrr %[out1],%[in1],%[in2],0x18" : [out1] "=r"(tempx) : [in1] "r"(gray), [in2] "r"(gray * gx_array[dy + 1][dx + 1]));
          asm volatile("l.nios_rrr %[out1],%[in1],%[in2],0x18" : [out1] "=r"(tempy) : [in1] "r"(gray), [in2] "r"(gray * gy_array[dy + 1][dx + 1]));
